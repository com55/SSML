...
class GameLauncherWorker(QThread):
    ...
    def run(self):
        game_exe = Path(self.config.GameExePath.get())
        mods_dir = Path(self.config.ModsDir.get())
        mod_ext = self.config.ModExtension.get()
        ...
        if restore:
            self.log_message("Waiting for game to close to restore files...")
...
class MainViewModel(QObject):
    ...
    def _ensure_defaults(self):
        if not self.config.ModsDir.get():
             default_mods = Path(os.getcwd()) / "Mods"
